...
public:
    ...
    void buy(double qty, string name, double price, string date) {
        ...
        //create a list of all prices and dates based on the quantity of the bought shares
        for(int i = 0; i < qty; i++){
            avgPurchases.add2back(Share(1, price, date));
        }
        totalShare = 0;
        totalCost = 0;
        typename List<Share>::Iterator it = avgPurchases.begin();
        while(!it.equal(avgPurchases.end())){
            Share &s = it.get();
            totalShare += s.qty;
            totalCost += s.price * s.qty;
            it.next();
        }
        saleAvgCostBasis = totalCost / totalShare; //calculate the average
    ...
    void sell(double qty, string name, double price, string date) {
        ...
        for(int i = 0; i < qty; i++){
            ...
            double gain = (price - saleAvgCostBasis);
            avgTax += gain * rate;
